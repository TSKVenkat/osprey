...
import { mediaAssets, recordings, storageConfigs, users } from '@bilby/db';
import { createTestDatabase } from '@bilby/db/testing';
import { LocalConnector, uploadLocalFile } from '@bilby/storage';
import { moovIsAtTheFront, probeFile } from '@bilby/processing';
...
describe('processRecording', () => {
  ...
  beforeEach(async () => {
    ...
    root = await mkdtemp(join(tmpdir(), 'bilby-worker-'));
  ...
  async function givenRecording(name: string, encodeArgs: string[]): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), 'bilby-input-'));
  ...
  it('salvages a recording whose last fragment is missing', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'bilby-truncated-'));
