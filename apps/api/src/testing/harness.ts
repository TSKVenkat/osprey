...
import type { Database } from '@bilby/db';
...
export async function createHarness(): Promise<Harness> {
  ...
  await reset(db);
  const storageRoot = await mkdtemp(join(tmpdir(), 'bilby-test-'));
  const env = loadEnv({
    NODE_ENV: 'test',
...
export async function login(
  ...
  const cookie = response.cookies.find((c) => c.name === 'bilby_session');
  if (!cookie) throw new Error('Login succeeded but set no session cookie.');
  return `bilby_session=${cookie.value}`;
}
