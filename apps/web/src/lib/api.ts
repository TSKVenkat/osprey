...
export interface RecordingDetail { recording: RecordingSummary & { description: string | null; sourceMime: string | null; width: number | null; height: number | null; hasAudio: boolean | null }; assets: { kind: string; bytes: number; contentType: string }[]; playback: { url: string; kind: string } | null; posterUrl: string | null; }
...
export const api = {
  ...
  listRecordings: (options: { cursor?: string; all?: boolean } = {}) => { const query = new URLSearchParams(); if (options.cursor) query.set('cursor', options.cursor); if (options.all) query.set('all', '1'); const suffix = query.size > 0 ? `?${query}` : ''; return request<{ recordings: RecordingSummary[]; nextCursor: string | null }>(`/v1/recordings${suffix}`); },
  getRecording: (id: string) => request<RecordingDetail>(`/v1/recordings/${id}`),
  renameRecording: (id: string, title: string) => request<{ recording: RecordingSummary }>(`/v1/recordings/${id}`, { method: 'PATCH', body: JSON.stringify({ title }) }),
  deleteRecording: (id: string) => request<void>(`/v1/recordings/${id}`, { method: 'DELETE' }),
  startRecording: (input: { title: string; mimeType: string; recordedWith?: unknown }) => request<StartedUpload>('/v1/recordings', { method: 'POST', body: JSON.stringify(input) }),
  completeUpload: (sessionId: string, options: { interrupted?: boolean } = {}) => request<{ recordingId: string; state: string }>(`/v1/uploads/${sessionId}/complete`, { method: 'POST', body: JSON.stringify(options) }),
  abortUpload: (sessionId: string) => request<{ ok: true }>(`/v1/uploads/${sessionId}/abort`, { method: 'POST' }),
  getUploadSession: (sessionId: string) => request<{ state: string; partSize: number; parts: { partNumber: number; bytes: number }[] }>(`/v1/uploads/${sessionId}`),
  createShare: (recordingId: string, input: { visibility: 'link' | 'password' | 'authenticated'; password?: string }) => request<{ share: ShareLink }>(`/v1/recordings/${recordingId}/shares`, { method: 'POST', body: JSON.stringify(input) }),
  listShares: (recordingId: string) => request<{ shares: ShareLink[] }>(`/v1/recordings/${recordingId}/shares`),
  revokeShare: (id: string) => request<void>(`/v1/shares/${id}`, { method: 'DELETE' }),
  getViews: (recordingId: string) => request<{ views: number; completions: number; totalWatchedMs: number }>(`/v1/recordings/${recordingId}/views`),
  getShared: (token: string) => request<SharedRecording>(`/v1/shares/${token}`),
  unlockShare: (token: string, password: string) => request<{ ok: true }>(`/v1/shares/${token}/unlock`, { method: 'POST', body: JSON.stringify({ password }) }),
  reportView: (token: string, progress: { sessionKey: string; watchedMs: number; maxPositionMs: number; completed: boolean }) => request<{ ok: true }>(`/v1/shares/${token}/views`, { method: 'POST', body: JSON.stringify(progress) }),
  listUsers: () => request<{ users: SessionUser[] }>('/v1/admin/users'),
  createUser: (input: { email: string; name: string; password: string; role: 'admin' | 'user' | 'viewer' }) => request<{ user: SessionUser }>('/v1/admin/users', { method: 'POST', body: JSON.stringify(input) }),
  listStorage: () => request<{ storage: StorageRow[] }>('/v1/admin/storage'),
  createStorage: (input: { kind: string; label: string; config: unknown; secret?: unknown; makeDefault?: boolean }) => request<{ storage: { id: string } }>('/v1/admin/storage', { method: 'POST', body: JSON.stringify(input) }),
  makeStorageDefault: (id: string) => request<unknown>(`/v1/admin/storage/${id}/default`, { method: 'POST' }),
