...
| **Name/trademark** — the project was called `openloom`, and "Loom" is a trademark | ~~High~~ Resolved | — | Renamed to `osprey` before going public. Checked against the names software already uses: `capybara`, `kestrel`, `falcon` and `frigate` are all taken, the last by a self-hosted video recorder |
