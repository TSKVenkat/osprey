...
describe('Cloudinary public ids', () => {
  ...
  it('prefixes an account folder when one is configured', () => {
    expect(publicIdFor('r/abc/original.mp4', 'osprey')).toBe('osprey/r/abc/original');
    expect(publicIdFor('r/abc/original.mp4', 'osprey/')).toBe('osprey/r/abc/original');
  });
...
describe('ImageKit paths', () => {
  ...
  it('prefixes an account folder', () => {
    expect(pathFor('r/abc/original.mp4', 'osprey')).toEqual({
      folder: '/osprey/r/abc',
      fileName: 'original.mp4',
    });
  });
  ...
  it('tolerates slashes around the configured folder', () => {
    expect(pathFor('r/abc/original.mp4', '/osprey/').folder).toBe('/osprey/r/abc');
  });
  ...
  it('rebuilds the full path for lookups and URLs', () => {
    ...
    expect(filePathFor('r/abc/original.mp4', 'osprey')).toBe('/osprey/r/abc/original.mp4');
